...
exports.submitContactForm = async (req, res) => {
  ...
  if (!name || !email || !subject || !message) {
    console.error("Error: Missing fields in the submission:", {
      name,
      email,
      subject,
      message,
  ...
  try {
    ...
    const verificationWorksheet =
      verificationWorkbook.getWorksheet("contact_us");
    console.log(
      "Data after updating, row count:",
      verificationWorksheet.rowCount
    );
    const lastRow = verificationWorksheet.getRow(
      verificationWorksheet.rowCount
    );
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Contact form data saved and SMS sent successfully" });
  } catch (error) {
